...
if ($set=='insert') {
        $pdo_insere = $conexao_pdo->prepare('INSERT INTO cliente (nome, telefone, email, senha, confirmesenha )
        VALUES (?, ?, ?, ?, ?)');
        $pdo_insere->execute(array($nomecompleto, $telefone, $email, $senha, $confirmesenha ));
        sets();
} elseif ($set == 'up') {
        //Executa alteração no registro do cliente
        $pdo_insere = $conexao_pdo->prepare("UPDATE cliente
                                             SET nome=?, telefone=?, email=?, senha=?, confirmesenha=?
                                             WHERE email=?");
        $pdo_insere->execute(array($nome, $telefone, $email, $senha, $confirmesenha ));
        sets();
} elseif ($set == 'del') {
